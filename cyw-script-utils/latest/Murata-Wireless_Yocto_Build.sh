...
echo "------------------------"
ORGA_FMAC="\""orga"\""
echo     "NOTE: $ORGA_FMAC is the only one currently available"
#echo     "1. "\""orga"\"" (v4.12)"
#echo -e  "2. battra - ${YLW}Will be available in future${NC}"

#echo -n -e "Select your entry: "
FMAC_VERSION="1"
#read FMAC_VERSION
	
	if [ "$FMAC_VERSION" = "1" ] && [ "$BRANCH_TAG_OPTION" = "y" ]; then
		FMAC_VERSION_STRING="Orga (4.12)"
		echo -e "${GRN}Selected : $ORGA_FMAC${NC}"
		echo " "
		echo -n "Hit any character to proceed: "
		read PROMPT
		...
		echo "|-----|------------------|---------|---------------|-----------------------|"
		echo "|  1  | 4.9.51 8MQ Beta  | morty   | 8             | imx8-morty-orga_r1.0  |"
		echo "|  2  | 4.9.11_1.0.0 GA  | morty   | 6,7           | imx-morty-orga_r1.1   |"
		echo "|  3  | 4.1.15_2.0.0 GA  | krogoth | 6,7 (No 7ULP) | imx-krogoth-orga_r1.0 |" 
		echo "----------------------------------------------------------------------------"
		break
...
if [ "$FMAC_VERSION" = "1" ]; then
	...
	if [ "$BRANCH_TAG_OPTION"    = "y" ] && [ "$ENTRY" = "1" ]; then
		...
		BRANCH_RELEASE_NAME=imx8-morty-orga_r1.0
		iMXYoctoRelease="4.9.51 8MQ Beta"
		YoctoBranch="morty"
		fmacversion="orga"
		metamuratawireless="imx8-morty-orga_r1.0"
	# morty-orga_r1.0
	elif [ "$BRANCH_TAG_OPTION"    = "y" ] && [ "$ENTRY" = "2" ]; then
		BRANCH_RELEASE_OPTION=2
		BRANCH_RELEASE_NAME=imx-morty-orga_r1.1
		iMXYoctoRelease="4.9.11_1.0.0 GA"
		YoctoBranch="morty"
		fmacversion="orga"
		metamuratawireless="imx-morty-orga_r1.1"
	# krogoth-orga_r1.0
	elif [ "$BRANCH_TAG_OPTION"    = "y" ] && [ "$ENTRY" = "3" ]; then
		BRANCH_RELEASE_OPTION=3
		BRANCH_RELEASE_NAME=imx-krogoth-orga_r1.0
		iMXYoctoRelease="4.1.15_2.0.0 GA"
		YoctoBranch="krogoth"
		fmacversion="orga"
		metamuratawireless="imx-krogoth-orga_r1.0"
	# ------------Developer Release--------------
	# imx8-morty
		...
		YoctoBranch="morty"
		fmacversion="orga"
		metamuratawireless="imx8-morty-orga"
	...
	# morty-orga
		...
		YoctoBranch="morty"
		fmacversion="orga"
		metamuratawireless="imx-morty-orga"
	...
	# krogoth-orga
		...
		YoctoBranch="krogoth"
		fmacversion="orga"
		metamuratawireless="imx-krogoth-orga"
	...
	if [ "$BRANCH_TAG_OPTION"    = "y" ] && [ "$ENTRY" = "1" ]; then
		BRANCH_RELEASE_OPTION=1
		BRANCH_RELEASE_NAME=imx8-morty-battra_r1.0
	# morty-battra_r1.0
	elif [ "$BRANCH_TAG_OPTION"    = "y" ] && [ "$ENTRY" = "2" ]; then
		BRANCH_RELEASE_OPTION=2
		BRANCH_RELEASE_NAME=imx-morty-battra_r1.0
	# krogoth-battra_r1.0
	elif [ "$BRANCH_TAG_OPTION"    = "y" ] && [ "$ENTRY" = "3" ]; then
		BRANCH_RELEASE_OPTION=3
		BRANCH_RELEASE_NAME=imx-krogoth-battra_r1.0
	# imx8-morty
	elif   [ "$BRANCH_TAG_OPTION" = "n" ] && [ "$ENTRY" = "1" ]; then
		BRANCH_RELEASE_OPTION=4
		BRANCH_RELEASE_NAME=imx8-morty-battra
	# morty-battra
	elif [ "$BRANCH_TAG_OPTION" = "n" ] && [ "$ENTRY" = "2" ]; then
		BRANCH_RELEASE_OPTION=5
		BRANCH_RELEASE_NAME=imx-morty-battra
	# krogoth-battra
	elif [ "$BRANCH_TAG_OPTION" = "n" ] && [ "$ENTRY" = "3" ]; then
		BRANCH_RELEASE_OPTION=6
		BRANCH_RELEASE_NAME=imx-krogoth-battra
	fi
fi
